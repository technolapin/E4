...
unsigned int
getHorizontalCoordinate(struct xvimage* img,
			unsigned int pixel_indice)
{
  return pixel_indice % img->row_size;
}
unsigned int
getVerticalCoordinate(struct xvimage* img,
			unsigned int pixel_indice)
{
  return pixel_indice/img->row_size % img->col_size;
}
...
struct graphe *initImageGraph(struct xvimage *image, 
  ...
  for(x_j = 0; x_j < cs; x_j++ )
	...
	y_i = x_i + tab_es_i[k];
	y_j = x_j + tab_es_j[k];
	if (y_i >= 0 && y_j >= 0 && y_i < rs && y_j < cs)
	  AjouteArc(G,
		    getVertex(image, x_i, x_j, 0, 0),
		    getVertex(image, y_i, y_j, 0, 0)
		    );
